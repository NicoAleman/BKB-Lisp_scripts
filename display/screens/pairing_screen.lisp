...
(defun pairing_screen (){
    (if (= firts_iteration_pair 0){
     ...
     (if (and (= pairing_key_R 127) (> signal_level -80)){
        ...
        (txt-block-l text_box_small 1 0 0  font_9x14 "SAVE")
        (disp-render text_box_small (+ x_offset 90) (+ y_offset 49) '(0 0xFFFFFF))
        (img-clear text_box_small)
     ...
     (if (= cfg_pressed_long 1) {
        ...
        (if (= pairing_key_R 127) { ; Only allow Save if pair is found
